...
namespace Mirror
{
    ...
    public class NetworkRoomManager : NetworkManager
    {
        ...
        public void ListenServer()
        {
            if (trsp == null)
            {
                if (TryGetComponent(out Transport newTrsp))
                {
                    trsp = newTrsp;
                }
                else
        ...
        {
            Debug.Log($"---NRM -CallOnClientEnterRoom");
            OnRoomClientEnter();
        ...
        {
            Debug.Log($"---NRM -CallOnClientExitRoom");
            OnRoomClientExit();
            foreach (NetworkRoomPlayer player in roomSlots)
                if (player != null)
                {
        ...
        {
            Debug.Log($"---NRM -CheckReadyToBegin");
            if (!Utils.IsSceneActive(RoomScene))
                return;
        ...
        {
            Debug.Log($"---NRM -OnClientConnect");
            roomPlayerParent = FindObjectOfType<Canvas>().transform.GetChild(1);//วาด็
            Instantiate(roomPlayerPrefab, roomPlayerParent);
            OnRoomClientConnect();
        ...
        {
            Debug.Log($"---NRM -OnClientDisconnect");
        ...
        {
            Debug.Log($"---NRM -OnClientSceneChanged");
            if (Utils.IsSceneActive(RoomScene))
            {
