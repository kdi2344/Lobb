...
namespace Mirror.Examples.EdgegapLobby
{
    public class UILobbyList : MonoBehaviour
    {
        ...
        public void Start()
        {
            Debug.Log($"---UILobbyList -Start");
            Refresh();
        }

        private void Refresh()
        {
            Debug.Log($"---UILobbyList -Refresh");
        ...
        {
            Debug.Log($"---UILobbyLists -Join");
            NetworkRoomManager.instance.networkAddress = lobby.lobby_id;
            NetworkRoomManager.instance.StartClient();
        }

        public void SetLobbies(LobbyBrief[] lobbies)
        {
            Debug.Log($"---UILobbyLists -SetLobbies");
            Loading.SetActive(false);
            Error.text = "";
